...
void HttpRequest::send(const QByteArray &data)
{
    ...
    if(methodName == "GET") {
        ...
        QNetworkReply* reply = netManager->post(d_ptr->getRequest(), data);
        d_ptr->setReply(reply);

    } else if(methodName == "PUT") {
        QNetworkReply* reply = netManager->put(d_ptr->getRequest(), data);
        d_ptr->setReply(reply);
...
void HttpRequest::send(QHttpMultiPart *multiPart)
{
    QNetworkAccessManager* netManager = d_ptr->getManager();
    if(netManager == Q_NULLPTR) {
        qDebug() << "NetAccessManager is nullptr";
    ...
    if(methodName == "POST") {
        QNetworkReply* reply = netManager->post(d_ptr->getRequest(), multiPart);
        d_ptr->setReply(reply);
