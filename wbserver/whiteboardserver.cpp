...
namespace wb {
    ...
    void WhiteboardServer::incomingConnection(qintptr sock)
    {
    ...
    void WhiteboardServer::onUserJoined(QByteArray name, int id)
    {
    ...
    void WhiteboardServer::onUserLeft(QByteArray name, int id)
    {
        QJsonDocument doc;
        QJsonObject rootObj;
        ...
        bool removed = false;
        m_clients.remove_if([=, &removed](ClientConnection*& c){
            if(id == c->id())
            {
                removed = true;
                c->write(jsonString);
        ...
        qDebug() << "WhiteboardServer::onUserLeft broadcast user_left";
        for(auto c : m_clients) c->write(jsonString);
    }

    void WhiteboardServer::onAddFigureReq(const QJsonObject &figure)
    {
    ...
    void WhiteboardServer::onDeleteFigureReq(int globalId)
    {
    ...
    void WhiteboardServer::onClearReq(int ownerId)
    {
        if(ownerId == -1)
        {
